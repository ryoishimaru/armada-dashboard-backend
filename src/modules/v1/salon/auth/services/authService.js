...
class authService {
  ...
  async signupService(requestData, requestQuery) {
    try {
      ...
      // fetch user email
      let userEmail = await this.authModel.fetchObjWithSingleRecord(
        wherQuery,
        'email',
      ...
      // fetch salon code
      let isSalonCode = await this.authModel.fetchObjWithSingleRecord(
        { salonCode },
        'salonCode',
        tableConstants.SALON
      );
      ...
      // fetch salon code
      const isValidSalon = await this.validateSalonCodeService(
        requestQuery.code.trim()
      );

      // if the salon code is invalid
      if (
        !(
          isValidSalon?.status_code === StatusCodes.OK &&
          isValidSalon?.code ===
            (await this.commonHelpers.getResponseCode('SALON_CODE_VALID'))
        )
      ) {
        return {
          status_code: StatusCodes.BAD_REQUEST,
  ...
  async loginService(requestData, requestHeader) {
    try {
      ...
      if (!userData) {
        ...
        return {
          status_code: StatusCodes.BAD_REQUEST,
          code: await this.commonHelpers.getResponseCode(
  ...
  async confirmSignupService(reqUser, res) {
    try {
      ...
      // fetch email
      let getUserEmail = await this.authModel.fetchObjWithSingleRecord(
        wherQuery,
        'email',
        tableConstants.SALON
      );
  ...
  async requestResetPasswordService(requestData) {
    try {
      ...
      const resetToken = this.JwtAuthSecurity.generateJwtToken({
  ...
  async resetPasswordService(reqUser, requestData) {
    try {
      ...
      const cryptedPassword = await this.passwordHash.cryptPassword(
        newPassword.trim()
      );

      await this.authModel.updateObj(

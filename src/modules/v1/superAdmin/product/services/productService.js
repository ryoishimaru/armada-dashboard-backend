    ...
    try {
      ...
      if (requestdata.productId) {
        ...
        const isProductExist =
          await this.saProductModel.fetchObjWithSingleRecord(
            { id: decodedProductId },
            'id',
        ...
        await this.saProductModel.updateObj(
          product_data,
          { id: decodedProductId },
          tableConstants.PRODUCT
        );
      ...
      const productId = await this.saProductModel.createObj(
        product_data,
        tableConstants.PRODUCT
      );
      ...
      if (!Array.isArray(requestFiles.image)) {
        ...
        await this.commonHelpers.uploadFileToSFTP(
          `${filePath}/${image.name}`,
        ...
        for (const file of requestFiles.image) {
          ...
          await this.commonHelpers.uploadFileToSFTP(
            ...
            `/file/ae_direct/${uploaded_file.name}`
          );
  ...
  async getProducts(reqQuery) {
    try {
      ...
      return await this.commonHelpers.prepareResponse(
        StatusCodes.OK,
        'SUCCESS',
        { productList, loadMore, count: productCount }
      );
    } catch (error) {
      this.logger.error(error);
      return error;
  ...
  */
  async deleteProduct(reqParams) {
    try {
      ...
      let isValidProductId = await this.saProductModel.fetchObjWithSingleRecord(
        where,
        'id',
        tableConstants.PRODUCT
      );
      ...
      // Delete the product
      await this.saProductModel.deleteObj(where, tableConstants.PRODUCT);
  ...
  */
  async getProductDetail(reqParams) {
    try {
      ...
      const [product] = await this.saProductModel.fetchProductDetails({
        'prod.id': productId,
      });
